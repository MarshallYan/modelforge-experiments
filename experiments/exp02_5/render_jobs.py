...
if __name__ == "__main__":
    ...
    range_number_of_radial_basis_functions = [128]
    range_per_atom_force = [0, 0.001, 0.1]

    count = 0
    for seed in range_seed:
        for number_of_radial_basis_functions in range_number_of_radial_basis_functions:
            for per_atom_force in range_per_atom_force:

                # config names
                experiment_name = f"02(5)_F{per_atom_force}_({seed})"
                project = "schnet_qm9"
                group = "exp02_5"
                tags = [
                    f"{seed=}",
